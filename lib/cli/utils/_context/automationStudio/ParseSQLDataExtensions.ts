export default (sqlQuery: string) => {
    ...
    const cleanedTableNames =
        ...
        tableNames.map((tableName) =>
            tableName.replace(/(?:FROM|JOIN)\s+/i, '').trim()
        );

    // Remove brackets from the table names
    const cleanedTableNamesWithoutBrackets =
        cleanedTableNames &&
        cleanedTableNames.map((tableName) =>
            tableName.replace(/(?:\[|\])/g, '')
        );

    // Exclude table names that begin with an underscore
    const filteredTableNames =
        cleanedTableNamesWithoutBrackets &&
        cleanedTableNamesWithoutBrackets.filter(
            (tableName) => !tableName.startsWith('_')
        );
